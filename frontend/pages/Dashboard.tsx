 ...
 * Supports both Student and Instructor dashboards
 */

import { Link, Navigate, useNavigate } from 'react-router-dom';
import { useQuery } from '@tanstack/react-query';
import { useAuth } from '../context/AuthContext';
import { instructorAPI, studentAPI, messagingAPI } from '../services';
...
const Dashboard = () => {
  ...
  if (!isInstructor) {
    ...
    // Error state for student dashboard
    if (studentError) {
      return (
    ...
    const handleBookAgain = (instructorId: number) => {
      // Navigate to instructor profile page where booking modal is available
      navigate(`/instructors/${instructorId}`);
    };

    const handleMessage = async (instructorId: number) => {
      try {
        // Start or get existing conversation with the instructor's user_id
        // Find the instructor to get their user_id
        const instructor = studentDashboard?.my_instructors.find(
    ...
    const handleCancelSession = async (sessionId: number) => {
      if (window.confirm('Are you sure you want to cancel this session?')) {
        try {
          await studentAPI.cancelSession(sessionId);
    ...
    const handleRescheduleSession = (sessionId: number) => {
      window.location.href = `/reschedule/${sessionId}`;
    };
      ...
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-white to-primary-50/30 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 py-8 relative">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 animate-fadeInUp">
            ...
            <Link
              to="/instructors"
              className="inline-flex items-center justify-center gap-2 px-6 py-3 bg-gradient-to-r from-primary-600 to-blue-600 text-white font-semibold rounded-xl hover:from-primary-500 hover:to-blue-500 transition-all shadow-lg shadow-primary-500/25 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] group"
            >
              Find a Tutor
          ...
          {studentDashboard && (
            <div className="mb-8 animate-fadeInUp" style={{ animationDelay: '0.1s' }}>
              <StudentStatsCards stats={studentDashboard.stats} />
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            ...
            <div className="lg:col-span-2 animate-fadeInUp" style={{ animationDelay: '0.2s' }}>
              <UpcomingSessionsCard
                sessions={studentDashboard?.upcoming_sessions || []}
            ...
            <div className="animate-fadeInUp" style={{ animationDelay: '0.25s' }}>
              <MyInstructorsSection
                instructors={studentDashboard?.my_instructors || []}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="animate-fadeInUp" style={{ animationDelay: '0.3s' }}>
              <SessionHistorySection
                sessions={studentDashboard?.session_history || []}
            ...
            <div className="animate-fadeInUp" style={{ animationDelay: '0.35s' }}>
              <PaymentHistorySection
                payments={studentDashboard?.payment_history || []}
              />
            </div>
          </div>
        </div>
